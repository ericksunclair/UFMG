...
class ponto2D {
    ...
    public:
        // contrutores
        ponto2D(): x(.0), y(.0), id(getNextId(1)){};    // construtor padrão
        ponto2D(double _x, double _y): x(_x), y(_y), id(getNextId(1)){};    // construtor com ambas as coordenadas
        ponto2D(const class ponto2D &that): x(that.x), y(that.y), id(getNextId(1)){};   // construtor por cópia usando parênteses
        ponto2D &operator=(const class ponto2D &that){
            x = that.x;
            y = that.y;
            id = getNextId(1);
            return *this;
