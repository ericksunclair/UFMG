...
unsigned int ponto2D::getNextId(int i){
    ...
    if(i==1){               // i==1 --> construtor (criar o Id do objeto que tá sendo criado)
        if(id_geral.size()==0){
    ...
    if(i==0){               // i==0 --> destrutor (excluir o Id do objeto que tá sendo excluído)
        id_geral.pop_back();    // aqui estou apagando o último Id e não o que eu deveria apagar
    }
    return id_geral.back();
}
